...
(define tests
  (test-suite
   ...
   (check-equal? (sequence 1 99 100) (list 1) "Sequence test 2")
   (check-equal? (sequence 3 1 1) null "Sequence test 3")
   ...
   (check-equal? (string-append-map 
                  (list "Iron" "Spider" "Bat") 
                  "Man") '("IronMan" "SpiderMan" "BatMan") "string-append-map test 2")
   
   (check-equal? (string-append-map 
                  null
                  "Man") null "string-append-map test 3")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero NATs) 5) (list (cons 0 1) (cons 0 2) (cons 0 3) (cons 0 4) (cons 0 5)) "stream-add-zero test")
